...
namespace nothinbutdotnetprep.specs
{
    public class MovieLibarySpecs
    {
        ...
        public class when_iterating_the_set : movie_library_concern
        {
            static IEnumerable<Movie> result;

            Establish c = () => { Enumerable.Range(1, 100).each(x => movie_collection.Add(new Movie())); };

            Because b = () => { result = sut.all_movies(); };

            It should_iterate = () => { result.GetEnumerator().MoveNext(); };
        }
        ...
        public class when_searching_for_movies : concern_for_searching_and_sorting
        {
            ...
            It should_be_able_to_find_all_kid_movies = () =>
            {
                var results = sut.all_movies().all_items_matching(
                    Where<Movie>.has_a(x => x.genre).equal_to(Genre.kids));
            ...
            {
                var results = sut.all_movies().all_items_matching(
                    Where<Movie>.has_a(x => x.genre).equal_to(Genre.action));
